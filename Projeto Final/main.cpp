...
float exibirPrecoAluguel(float tamanhoCTA, int diasAluguel){
  ...
  if (tamanhoCTA <= 2) {
    if (diasAluguel > 0 && diasAluguel <= 15){
      precoAluguel = (tamanhoCTA * 15) * 5 + 12;
    } else if (diasAluguel > 15 && diasAluguel <= 30) {
        precoAluguel = (tamanhoCTA * 30) * 4.5 + 12;
      } else if (diasAluguel > 30 && diasAluguel <= 60) {
          precoAluguel = (tamanhoCTA * 40) * 4.5 + 12; }
  } 
  
  // preço aluguel tamanho 3 ao 4
  else if (tamanhoCTA > 2 && tamanhoCTA <= 4) {
    if (diasAluguel > 0 && diasAluguel <= 15) {
      precoAluguel = (tamanhoCTA * 15) * 4.5 + 12;
    } else if (diasAluguel > 15 && diasAluguel <= 30) {
        precoAluguel = (tamanhoCTA * 30) * 4 + 12;
      } else if (diasAluguel > 30 && diasAluguel <= 60)     { precoAluguel = (tamanhoCTA * 40) * 3.5 + 12; }
  }

  // preço aluguel tamanho 5 ao 7
  else {
    if (diasAluguel > 0 && diasAluguel <= 15) {
      precoAluguel = (tamanhoCTA * 15) * 4 + 12;
    } else if (diasAluguel > 15 && diasAluguel <= 30) {
        precoAluguel = (tamanhoCTA * 30) * 3 + 12;
      } else if (diasAluguel > 30 && diasAluguel <= 60)     { precoAluguel = (tamanhoCTA * 40) * 2.5 + 12; }
  }
...
void exibirFormaDePagamento(float precoAluguel) {
  int escolhaFormaDePagamento, escolhaCartaoDeCredito;
  ...
  switch (escolhaFormaDePagamento) {
    case 1:
      ...
      cout << "     Digite o número correspondente à sua escolha: ";
      cin >> escolhaCartaoDeCredito;
      } while (escolhaCartaoDeCredito < 0 || escolhaCartaoDeCredito > 3);
  
      switch (escolhaCartaoDeCredito) {
          case 1:
            novoPrecoAluguel = precoAluguel;
            
            cout << "\n     Total\t-\t1x de R$ " << novoPrecoAluguel;
            break;

          case 2:
            novoPrecoAluguel = precoAluguel /2;
            
            cout << "\n     Total\t-\t2x de R$ " << novoPrecoAluguel;
            break;

          case 3:
            novoPrecoAluguel = precoAluguel /3;

            cout << "\n     Total\t-\t3x de R$ " << novoPrecoAluguel;
            break;
      }
      break;
    ...
    case 2:
      ...
      cout << "\n     Você escolheu a opção de pagar com cartão de débito, ganhando, portanto, um desconto de 3%. Sendo assim, o valor passou de R$ " << precoAluguel << " para R$ " << novoPrecoAluguel;
      break;
    
    case 3:
      novoPrecoAluguel = precoAluguel * 0.95;

      cout << "\n     Você escolheu a opção de pagar com PIX, ganhando, portanto, um desconto de 5%. Sendo assim, o valor passou de R$ " << precoAluguel << " para R$ " << novoPrecoAluguel;
      break;
  }
...
int main() {
  ...
  // Lista de variáveis
  std::string nome; // variável para nome do(a) cliente
  std::string ultimoSobrenome; // variável para o último sobrenome do(a) cliente
  std::string email; // variável para o e-mail do(a) cliente
  int menuPrincipal, comprimentoDog, larguraDog, alturaDog, cabineOuPorao, caixaOuBolsa, estadoPartida, estadoDestino, simNaoFAQs, diasAluguel, estadoDevolucao; // 1. variável para opção no menu principal - 2. comprimento do doguinho - 3. largura do doguinho - 4. altura do doguinho - 5. opção de viagem na cabine ou no porão - opção de caixa ou bolsa de transporte - 6. estado de partida selecionado - 7. estado de destino selecionado - 8. dias que a CTA ficará alugada
  float pesoDog, tamanhoCTA, precoAluguel, novoPrecoAluguel; // 1. peso do doguinho - 2. tamanho da caixa - 3. preço do aluguel da CTA - 4. Preço do aluguel após escolha da forma de pagamento
  char estadosUF[27][3] = {"RO", "AC", "AM", "RR", "PA", "AP", "TO", "MA", "PI", "CE", "RN", "PB", "PE", "AL", "SE", "BA", "MG", "ES", "RJ", "SP", "PR", "SC", "RS", "MS", "MT", "GO", "DF"}; // vetor de strings (palavras) com as siglas de cada estado
  ...
  cout << "\n_______________________________________________________________________";
    ...
    switch (menuPrincipal) {
      case 1:
        ...
        cout << "\n     Digite o número correspondente ao estado de partida: ";
      ...
      do{  
        ...
        switch (estadoDevolucao) {
          case 1:
            ...
            cout << "\n     Digite o número correspondente ao estado de destino: ";
